...
use near_sdk::{env, ext_contract, PromiseOrValue, assert_one_yocto, Gas, PromiseResult, log, Promise, NearToken};

pub const STORAGE_BALANCE_MIN_BOUND: NearToken = NearToken::from_yoctonear(1_250_000_000_000_000_000_000u128);
const GAS_FOR_RESOLVE_TRANSFER: Gas = Gas::from_tgas(10);
const GAS_FOR_MFT_TRANSFER_CALL: Gas = Gas::from_tgas(30);
...
impl Contract {
    ...
    pub fn mft_register(&mut self, token_id: String, account_id: AccountId) {
        ...
        let refund = if token.accounts.contains_key(&account_id) {
            ...
            amount.checked_sub(STORAGE_BALANCE_MIN_BOUND).unwrap()
        };
    ...
    pub fn mft_transfer_call(
        ...
        ext_mft_receiver::ext(receiver_id.clone())
            .with_static_gas(env::prepaid_gas().checked_sub(GAS_FOR_MFT_TRANSFER_CALL).unwrap().checked_sub(GAS_FOR_RESOLVE_TRANSFER).unwrap())
            .mft_on_transfer(token_id.clone(), sender_id.clone(), amount, msg)
            .then(
                Self::ext(env::current_account_id())
                    .with_static_gas(GAS_FOR_RESOLVE_TRANSFER)
                    .mft_resolve_transfer(token_id, sender_id, receiver_id, amount),
            ).into()
    }
