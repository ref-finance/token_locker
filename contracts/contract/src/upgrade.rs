...
impl Contract {
    ...
    pub fn migrate_state() -> Self {
        ...
        contract.data = match contract.data {
            VersionedContractData::V1000(data) => VersionedContractData::V1001(data.into()),
            VersionedContractData::V1001(data) => VersionedContractData::V1001(data),
        };
        contract
    }
...
mod upgrade {
    ...
    const GAS_TO_COMPLETE_UPGRADE_CALL: Gas = Gas::from_tgas(10);
    const GAS_FOR_GET_CONFIG_CALL: Gas = Gas::from_tgas(5);
    const MIN_GAS_FOR_MIGRATE_STATE_CALL: Gas = Gas::from_tgas(10);
    ...
    pub extern "C" fn upgrade() {
        ...
        unsafe {
            ...
            let required_gas =
                env::used_gas().checked_add(GAS_TO_COMPLETE_UPGRADE_CALL).unwrap().checked_add(GAS_FOR_GET_CONFIG_CALL).unwrap();
            require!(
                env::prepaid_gas() >= required_gas.checked_add(MIN_GAS_FOR_MIGRATE_STATE_CALL).unwrap(),
                "Not enough gas to complete state migration"
            );
            let migrate_state_attached_gas = env::prepaid_gas().checked_sub(required_gas).unwrap();
            ...
            sys::promise_batch_action_function_call(
                ...
                empty_args.as_ptr() as _,
                0 as _,
                GAS_FOR_GET_CONFIG_CALL.as_gas(),
            );
